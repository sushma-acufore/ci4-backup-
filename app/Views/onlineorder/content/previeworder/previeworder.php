  ...
  <div class="row ">
                <div class="col-lg-12 table-preview-area-row" style="padding: 10px;">
                        ...
                        <table class="table table-preview-data" id="table-data">
                            <thead>
                                <tr class="table-dark tr-head table-default-preview">
                                    ...
                                    <th class="tr-preview">Contact number : <br><?php if($json_data1['C_PHONE']!=''){ echo $json_data1['C_PHONE']; }else{ echo $_SESSION['E_PHONE'];} ?></th>
                                    <th class="tr-preview"> Email : <br><?php if($json_data1['C_EMAIL']!=''){ echo $json_data1['C_EMAIL']; }else{ echo $_SESSION['E_EMAIL'];} ?>
                                    </th>
                                </tr>
                            </thead>
                            ...
                            <?php
								...
								if(!empty($result1))
								{
                                    ...
                                    foreach ($result1 as $row1)
								    {
                                        ...
                                        $num_disc_items=Ordermodel::getproduct_cart_discount_items_count($rid);
                                        $fetch_disc_items = Ordermodel::getproduct_cart_discount_items($rid);
                                        ...
                                        $row_avl = Ordermodel::geteagle_available_stock_for_preview($product);
								        $avl_stock =0;
								        $serial_no_count = 0;
								        if(!empty($row_avl))
								        {
								        ...
								        $e1 .='<tr class="table-default-preview">
                                                ...
                                                {
                                                    $e1 .= '<td class="align-right"><button type="button" class="cart-table-btn-dealer-price" onclick=\'stock_dealer_alert("'.$product.'","'.$row1['sp_price'].'","'.$row1['stock_dealer_discount'].'","previeworder")\' >Click&nbsp;Here</button></td>';
                                                    $e1 .= '<td class="align-right">$'.$totalprice.'</td>';
                                                }
								                $e1 .='
								            </tr>';   
								            	...
								            	foreach ($fetch_disc_items as $fetch_disc_item_row) 
                                                    {
                                                        ...
                                                        $e1 .='<tr class="table-default-preview">
                                                                    <td data-toggle="tooltip" title="'.''.$full_discount_item_name.''.'">'.$discount_item_name.'</td>
                            ...
                            <tfoot>
                                <tr>
                                   <td colspan="6" rowspan="3" class="tax-col details" style="text-align: left;"> 
                                    ...
                                    <td style="float:right"><button class="btn-more-price" style="text-align: right;">$<?php echo number_Format($json_data1['SO_TAX'],2); ?></button></td>
                                </tr>
                    ...
                    <div class="row">
                        ...
                        <a href="cart"><button class="btn cancel-btn" type="button">Edit Order</button></a>

                            <!-- pop up -->
                            <!-- Button trigger modal -->
                            <form method="post" action="<?php echo base_url()?>billprint" id="myForm" style="float:right">
