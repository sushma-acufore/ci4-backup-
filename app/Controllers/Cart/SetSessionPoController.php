...
class SetSessionPoController extends BaseController
{
  ...
  public function set_session_po()
  {
      ...
      {
          $po_by_customer= trim($_POST['po_by_customer']);
          ...
          curl_close($ch1);
          //var_dump($json_data1);
          if(isset($json_data1["value"][0]['PO_NO'])){
            $ebms_po_no=$json_data1["value"][0]['PO_NO'];
            if($po_by_customer==$ebms_po_no){
                $response=1;
            }else{
                $_SESSION['po_by_customer'] = trim($_POST['po_by_customer']);
                $response=0;
            }
          }else{
            $_SESSION['po_by_customer'] = trim($_POST['po_by_customer']);
    ...
    public function set_session_po_newship()
    {
        if($_POST['po_by_customer'])
        {
            $po_by_customer= trim($_POST['po_by_customer']);
    ...
    public function unset_cart_sessions()
    {
